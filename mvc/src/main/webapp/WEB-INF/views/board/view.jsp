...
<script type="text/javascript">
	...
	function deleteRpChk(re_no, brd_no, pageNum) {
		if(confirm("정말 삭제하시겠습니까?")) {
			location.href="deleteReply?re_no="+re_no+"&brd_no="+brd_no+"&pageNum="+pageNum;
		}else {
			return;
		}
	}
	...
	/* function delChk() {
		...
		if(replyCount > 0 ) {
			...
			if(refTotal > 0) {
				...
				location.href="deleteForm?brd_no=${board.brd_no}&pageNum=${pageNum}";				
			}
		}
	} */
				...
				<div class="panel-heading">
					<div class="row">
						...
						<div class="col col-xs-6 text-right">
							<a href="board?pageNum=${pageNum}&searchType=${searchType}&searchTxt=${searchTxt}" class="btn btn-sm btn-default">목록<em class="fa fa-list-ul"></em></a>
							<c:set var="refLimit" value="${bs.refLimit(board.ref)}"></c:set>
							<c:if test="${sessionScope.no != null && board.re_level == 0 && refLimit < 6}">
								<a href="writeForm?brd_no=${board.brd_no}&pageNum=${pageNum}" class="btn btn-sm btn-default">답변<em class="fa fa-comment-o"></em></a>
							</c:if>
							<c:if test="${sessionScope.no == board.m_no}">
								<a href="updateForm?brd_no=${board.brd_no}&pageNum=${pageNum}" class="btn btn-sm btn-default">수정<em class="fa fa-pencil"></em></a>
							...
							<tr>
								...
								<td class="text-left">
									<c:forEach var="file" items="${fileList}">
				                        <a href="filedown?fileName=${file.f_stored_name}" class="fileview"><font size="2px">${file.f_original_name}</font></a> 
		...
		<div class="col-lg-9 col-md-offset-18">
			<div class="panel panel-default panel-table">
					...
					<div class="row">
						...
						<div class="replym_w">
							<div class="testimonials">
								...
								<div class="up">
									<blockquote>
										<form action="updateReply" id="upfrm" name="upfrm">
				...
				<div class="container re_bottom">
					<div class="row">
						<div class="replym_w">
							<form action="writeReply" name="refrm" onsubmit="return rpSubmit(${sessionScope.no})">
